...
struct Env* env_create(char* user_program_name, unsigned int page_WS_size,
	...
	pushcli();
	{
...
#else
		{
			...
			ptr_user_page_directory = STATIC_KERNEL_VIRTUAL_ADDRESS(
					to_physical_address(p));
		...
		if (percent_WS_pages_to_remove == 0)// If not entered as input, 0 as default value
			e->percentage_of_WS_pages_to_be_removed =
			DEFAULT_PERCENT_OF_PAGE_WS_TO_REMOVE;
		else
			e->percentage_of_WS_pages_to_be_removed =
					percent_WS_pages_to_remove;
		...
		PROGRAM_SEGMENT_FOREACH(seg, ptr_program_start) {
			...
			LOG_STATMENT(
					cprintf(
							"SEGMENT #%d, size_in_file = %d, size_in_memory= %d, dest va = %x",
							segment_counter, seg->size_in_file,
			...
			LOG_STATMENT(
					cprintf("SEGMENT: allocated pages in WS = %d",
							allocated_pages));
			LOG_STATMENT(
			...
			if (offset_first_page) {
				...
				if (pf_add_env_page(e, start_first_page,
					...
					panic(
							"ERROR: Page File OUT OF SPACE. can't load the program in Page file!!");
			...
			for (i = end_first_page; i < start_last_page;
					i += PAGE_SIZE, src_ptr += PAGE_SIZE) {
		...
		LIST_FOREACH(wse, &(e->page_WS_list))
		{
			...
			get_page_table(e->env_page_directory, virtual_address,
					&ptr_page_table);
		...
		for (; stackVa >= ptr_user_stack_bottom; stackVa -= PAGE_SIZE)
		{
			...
			allocate_frame(&pp);
			loadtime_map_frame(e->env_page_directory, pp, stackVa,
			...
			//now add it to the working set and the page table
			{
				...
				if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
					LIST_REMOVE(&(e->PageWorkingSetList),
							&(e->ptr_pageWorkingSet[lastWSIndex]));
					//Now: we are sure that at least the top page in the stack will be added to Active list
					//Since we left 1 empty location in the Active list when we loaded the program segments
					if (LIST_SIZE(&(e->ActiveList)) < e->ActiveListSize) {
						LIST_INSERT_HEAD(&(e->ActiveList),
								&(e->ptr_pageWorkingSet[lastWSIndex]));
					} else {
						LIST_INSERT_HEAD(&(e->SecondList),
								&(e->ptr_pageWorkingSet[lastWSIndex]));
					}
				}
#endif
		...
		if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
			...
			LIST_FOREACH(elm, &(e->SecondList))
			{
				...
				pt_set_page_permissions(e->env_page_directory,
						elm->virtual_address, 0, PERM_PRESENT);
			}
		}
...
void env_free(struct Env *e) {
	...
	while (!LIST_EMPTY(&e->page_WS_list)) {
		ele = LIST_FIRST(&e->page_WS_list);
		unmap_frame(e->env_page_directory, ele->virtual_address);
		LIST_REMOVE(&e->page_WS_list, ele);
		kfree(ele);
	...
	for (uint32 i = 0; i < e->num_heap_blocks; i++) {
		if (e->heap_blocks[i] != NULL) {
			kfree(e->heap_blocks[i]);
			e->heap_blocks[i] = NULL;
		}
	}
	e->num_heap_blocks = 0;
	...
	for (uint32 i = 0; i < PTSIZE / PAGE_SIZE; i++) {
		if (e->env_page_directory[i] & PERM_PRESENT) {
			uint32 va = i << 22;
			pd_clear_page_dir_entry(e->disk_env_tabledir, va); // 1 << 22 == i * 4mb
	...
	struct FrameInfo *pd_frame = get_frame_info(e->env_page_directory, va,
			&ptr_page_table);
	if (pd_frame != NULL) {
		free_frame(pd_frame);
	}
...
void sched(void) {
	...
	if (!holding_kspinlock(&ProcessQueues.qlock))
		panic(
				"sched: q.lock is not held by this CPU while it's expected to be. ");
...
// Initialize the TSS field of the gdt.
	c->gdt[GD_TSS >> 3] = SEG16(STS_T32A, (uint32 ) (&(c->ts)),
...
//In [tpp or tia's]: Load max of 9 pages only for the data segments that start with va = 803000
	if (iVA == 0x803000
			&& (strcmp(e->prog_name, "tpp") == 0
					|| strcmp(e->prog_name, "tia") == 0
					|| strcmp(e->prog_name, "tia_slave1") == 0
					|| strcmp(e->prog_name, "tia_slave2") == 0
					|| strcmp(e->prog_name, "tia_slave3") == 0
					|| strcmp(e->prog_name, "tia_slave4") == 0))
	...
	for (; iVA < end_vaddr && i < remaining_ws_pages; i++, iVA += PAGE_SIZE) {
		...
		loadtime_map_frame(e->env_page_directory, p, iVA,
		PERM_USER | PERM_WRITEABLE);
...
#else
			LIST_REMOVE(&(e->PageWorkingSetList),
					&(e->ptr_pageWorkingSet[e->page_last_WS_index]));
			//Always leave 1 page in Active list for the stack
			if (LIST_SIZE(&(e->ActiveList)) < e->ActiveListSize - 1) {
				LIST_INSERT_HEAD(&(e->ActiveList),
						&(e->ptr_pageWorkingSet[e->page_last_WS_index]));
		...
		if (PDX(iVA) != (*lastTableNumber)) {
			e->__ptr_tws[e->table_last_WS_index].virtual_address = ROUNDDOWN(
					iVA, PAGE_SIZE*1024);
			;
...
//return a pointer to the start of the allocated space (including the GUARD PAGE)
	...
	if (user_kern_stack == NULL)
		panic("Cant allocate user kernel stack");

	uint32 guard_viad = (uint32) user_kern_stack;
	uint32 * ptr_pg_table = NULL;
	get_page_table(ptr_user_page_directory, guard_viad, &ptr_pg_table);
	ptr_pg_table[PTX(guard_viad)] = ptr_pg_table[PTX(guard_viad)]
			& ~PERM_PRESENT;
	return (void*) guard_viad;
}
...
// Allocate the page working set
...
#else
	{
		...
		e->__uptr_pws = (struct WorkingSetElement*) (((struct Env*) (UENVS
								+ sizeof(struct Env) * env_index))->ptr_pageWorkingSet);
	}
#endif
...
// Add its elements to the "e->PageWorkingSetList"
	if (isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX)) {
		...
		for (int i = 0; i < e->page_WS_max_size; ++i) {
			LIST_INSERT_HEAD(&(e->PageWorkingSetList),
					&(e->ptr_pageWorkingSet[i]));
		}
#endif
	}
...
// Generate an env_id for this environment.
	/*2022: UPDATED*/generation = (e->env_id + (1 << ENVGENSHIFT))
			& ~(NEARPOW2NENV - 1);
